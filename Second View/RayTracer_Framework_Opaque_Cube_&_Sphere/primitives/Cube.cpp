...
Intersect_Cond Cube::intersection_check(const M3DVector3f start, const M3DVector3f dir, float &distance, M3DVector3f intersection_p)
{
...
void	Cube::shade(M3DVector3f view, M3DVector3f intersect_p, const Light & sp_light, M3DVector3f am_light, M3DVector3f color, bool shadow)
{
	//Decrease shininess to 15 to 20 for plastic paint colors
	//Used 100 just to show shading effects in cubes
	float shininess_constant = 150;
...
void	Cube::get_reflect_direct(const M3DVector3f direct, const M3DVector3f intersect_p, M3DVector3f reflect_direct)
{
	M3DVector3f normal,_pos;
	m3dLoadVector3(_pos, 1, 1, 1);
...
bool	Cube::get_refract_direct(const M3DVector3f direct, const M3DVector3f intersect_p, M3DVector3f refract_direct, float delta, bool is_in)
{
	M3DVector3f n_direct, _pos;
	m3dLoadVector3(_pos, 1, 1, 1);
	m3dCopyVector3(n_direct, direct);
	m3dScaleVector3(n_direct, -1);
	...
	if (R >= 0)
	{
		m3dScaleVector3(normal, Sc * prod_n_d - R);
...
void Cube::load_texture(std::string file_name)
{
	Image img_texture;
	img_texture.nx = 256;
	img_texture.ny = 256;
...
void Cube::texture_color(M3DVector3f pos, M3DVector3f color)
{

}

void	Cube::get_texel(float x, float y, M3DVector3f color)
{
	return;
}